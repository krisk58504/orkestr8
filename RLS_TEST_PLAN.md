...
npx tsx scripts/run-sql.ts supabase/tests/rls_phase7_notifications.sql
...
| 2026-05-26 | `scripts/run-sql.ts` (pg) | **20 / 20 suites pass — 286 / 286 cumulative** | Phase 7 slice 1 walk-test sign-off. Pre-existing `is_ai_actor()` helper bug fixed in migration `20260610000000_fix_is_ai_actor_empty_string.sql` — pgbouncer Session pooling leaves `app.is_ai_actor=''` after rolled-back `set_config` calls, which caused `''::boolean` to raise SQLSTATE 22P02 intermittently in Suites 16 and 19. `nullif(_, '')` wrap repairs the empty-string path; behavior unchanged for `'true'`/`'false'`. Determinism verified by 2 back-to-back full-suite runs (20/20 + 20/20) with the pool probe confirming `app.is_ai_actor=''` between runs — i.e. the fix succeeds **on the polluted pool**, not just by luck. |
| 2026-05-27 | (pending walk-test) | 8 expected | `rls_phase7_notifications.sql` — Suite 21 — NX1 (PM-A SELECT own), NX2 (PM-A cross-org isolation), NX3 (recipient UPDATE own), NX4 (cross-user UPDATE denied), NX5 (client INSERT denied — no policy), NX6 (recipient DELETE own), NX7 (privileged INSERT for TENANT-role recipient mirrors producer admin-client path), NX8 (CASCADE on org delete). Application-layer actor-self-skip + zero-recipient skip are walk-test responsibilities (§8 of slice 2 audit). Phase 7 slice 2 substrate. |
| 2026-05-27 | `scripts/run-sql.ts` (pg) | **21 / 21 suites pass — 294 / 294 cumulative** | Phase 7 slice 2 walk-test sign-off. Slice 2 walk-test scenarios 1, 2, 3, 5, 6 verified manually on dev.orkestr8.ai; scenario 4 (cross-org isolation) covered by Suite 21 NX2; scenario 7 (recipient zero-found resolver edge case) deferred — hard to trigger naturally without an org-with-zero-managers fixture. Suite 21 NX8 originally tested live org delete to verify CASCADE; that path collides with `protect_user_columns` trigger (users.organization_id is ON DELETE SET NULL, trigger blocks the resulting users UPDATE). Switched to a structural `pg_constraint.confdeltype = 'c'` check — verifies the same cascade contract without triggering the user-column protection. |
| 2026-05-27 | `scripts/run-sql.ts` (pg) | **21 / 21 suites pass — 294 / 294 cumulative** (unchanged from slice 2 baseline) | Phase 7 slice 3 walk-test sign-off. Slice 3 added NO new RLS suite per audit §6 — handler uses admin client; existing `rent_charges` policies + slice 1 `automations` / `automation_runs` suites cover all surfaces. Walk-test verified all 5 ship-gate scenarios green: schema verification (no migration), automations row seeded (opt-in per discipline #9), cron returned 200 with idempotency working (15 leases eligible, 15 skipped already_charged), automation_runs row written correctly, **refactor verification: existing "Generate for property" button-triggered path produces identical output to slice 3 cron handler** (both use shared `periodForMonth` at `src/lib/automation/lib/periods.ts` — bit-identical refactor confirmed). One in-flight observation: discovered Sterling's `automation_freeze` was still `true` from slice 1 walk-test; cleared via `/settings/automations` UI before slice 3 curl. Documented for partner-facing follow-up: provisioning code should default `automation_freeze=false` on new orgs (it already does — the column default — but the staleness from a prior walk-test is the surfaced gap, not the default). |
| 2026-05-27 | `scripts/run-sql.ts` (pg) | **21 / 21 suites pass — 294 / 294 cumulative** (unchanged from slice 3 baseline) | Phase 7 slice 4 walk-test sign-off — Late Fee Handler (§3.20). Slice 4 added NO new RLS suite per audit §6.4 — handler uses admin client; new `parent_charge_id` self-FK column is covered by existing `rent_charges` per-row policies (Phase 5 slice 10a + 10e + Phase 6 slice 11a RESTRICTIVE no-AI-writes). Walk-test verified Step 0 (migration `20260612000000` applied + column + FK ON DELETE SET NULL + partial index verified) + all 8 scenarios (a-h) + cross-org Layer A + Layer B + EXPLAIN ANALYZE + cumulative regression. Two cross-slice observations: (1) `automation_freeze` stale on Kristophers from a prior walk-test re-surfaced the slice 3 §F.2 #2 discipline — pre-flight freeze check belongs in every cross-org walk-test. (2) EXPLAIN ANALYZE at Sterling's 61-row scale chose Seq Scan over the new partial index `rent_charges_parent_charge_id_idx` — acceptable per audit §9.2.5 contingency; production scale flips the planner's cost calculus. **Audit-commit timing note**: the slice 4 audit document was committed as `63f5df7` AFTER implementation + walk-test commits had pushed, breaking the slice 1/2/3 pre-implementation pattern. Documented in slice 4 §F.4; slice 5+ reverts to the pre-implementation audit-commit discipline. |
| 2026-05-27 | `scripts/run-sql.ts` (pg) | **21 / 21 suites pass — 294 / 294 cumulative** (unchanged from slice 4 baseline) | Phase 7 slice 5 walk-test sign-off — Insurance Certificate Renewal Cascade (#39). Slice 5 added NO new RLS suite per audit §6.2 — the migration `20260613000000_phase7_slice5_vendor_documents_expires_on_idx.sql` adds only a partial index `vendor_documents(organization_id, expires_on) WHERE expires_on IS NOT NULL`, a planner artifact with no row-access surface; the handler uses the admin client and existing `vendor_documents` policies + slice 1 `automations`/`automation_runs` suites cover all surfaces. Walk-test verified Step 0 (migration applied + index verified + slice 1 substrate intact + EXPLAIN ANALYZE both detection queries + cross-org freeze pre-check) + all 10 scenarios (a-j): cascade with 4 distinct per-threshold subjects (a), same-day idempotency (b), cross-threshold re-fire Case A/B (c), out-of-bounds filter (d), boundary crossing (e), `document_type='insurance'` isolation vs slice 1 (f), no-recipient skip path (g), runner-level freeze gate (h), cross-org Layers A+B with 100% org-match (i), cumulative regression (j). **Audit erratum**: audit §8.0 step 3 originally stated "4 policies" for vendor_documents; actual count is 2 (`vendor_documents_select` + `vendor_documents_write` per `20260519000800_phase2_rls.sql:70+79`) — corrected in commit `637cd93` after Step 0.3 verification; not a regression (slice 5 added 0 policies). **Cross-cutting follow-up surfaced**: the handler maps every non-delivered `sendEmail()` outcome (dedup-`suppressed`, allowlist-`blocked`) to `automation_runs.status='failed'` — observed 3× in walk-test; dedup-suppression will occur in production and surface as false-alarm `failed` runs. Recommended granular run-status mapping (`suppressed`/`blocked` distinct from `failed`) as part of a dedicated "slice 1 hardening" follow-up slice (also bundling the slice-1/slice-5 insurance double-email coordination). Audit-commit timing restored to the slice 1/2/3 pre-implementation pattern (`c57da82` + `b45ed36` before any code commit), reverting the slice 4 §F.4 anomaly. |
| 2026-05-28 | `scripts/run-sql.ts` (pg) | **21 / 21 suites pass — 294 / 294 cumulative** (unchanged from slice 5 baseline) | Phase 7 slice 6 walk-test sign-off — Slice 1 hardening (run-status taxonomy + insurance double-email coordination). Slice 6 added NO new RLS suite — the migration `20260614000000_phase7_slice6_automation_runs_status_taxonomy.sql` is a DROP+ADD CHECK extending `automation_runs.status` from `('running','ok','failed','skipped')` to add `'suppressed'`+`'blocked'`, a write-validation predicate with no row-access surface; the handlers use the admin client; work-stream B's `exclude_document_types` config field + slice-1 NOT-IN filter are query-shape changes, not policy changes; `automation_runs`/`automations` policies untouched. Suites 19/20 (`rls_phase7_automations*`) + 13 (`rls_phase2` vendor_documents) cover all surfaces. Walk-test verified Step 0 (constraint-name confirm `automation_runs_status_check` + pre-existing 13-row survival of the `ADD CONSTRAINT` + new 6-value domain) + Step 0.5 (**finding #4 mitigation** — insert-probe lands all 6 status values with no 23514, proving the DROP+ADD left no surviving narrow constraint) + scenarios a-j: suppressed→`'suppressed'`/`degraded` not `failed` (a), **non-vacuous OWNER-notification proof** via a temp walk-test OWNER — suppression fired 0 `automation_run.failed` notifications (a-addendum), genuine invalid_config failure→`'failed'`+OWNER notif 0→1 (e, the no-over-correction mirror), the `degraded`/`ok` derivation boundary pair (f: nothing-eligible→`ok` vs all-blocked→`degraded`, discriminator `(suppressed>0 OR blocked>0)`), no-recipient still `'skipped'` (g), non-email handlers derive `ok` never `degraded` (h), Item B double-email reproduced then fixed by `exclude_document_types:['insurance']` with w9 narrowness check (i), cumulative regression (j). **Finding #1 confirmed**: the third reader of `result.failed` (`runner.ts:135` OWNER-notification guard — the loudest false-alarm channel) now fires only on genuine failure; it self-corrects (no code change) once suppressed/blocked stop incrementing `failed`. **New signal**: `'degraded'` on `automations.last_run_status` (free text, no CHECK, no migration, no consumers — write-only at `runner.ts:118`). **Blocked-is-prod-reality** (audit §G.8): until EMAIL_SAFETY prod-email sign-off, Gate 3 blocks all prod sends so `blocked` is the dominant real outcome — which is why `degraded` matters. Audit-commit timing honored (`a0cb5cd` audit before all 5 implementation commits). Fixture cleanup single-tx: slice-1 config RESTORED (priority), walk-test rows deleted, `email_log` 26 rows preserved (org FK only). |
